...
void removeBefore(List *&L, int val)
{
    ...
    NODE* current = L->p_head;
    while (current->p_next && current->p_next->p_next && current->p_next->p_next->key != val) {
        current = current->p_next;
    }
    if (current->p_next && current->p_next->p_next) {
        NODE* temp = current->p_next;
        current->p_next = temp->p_next;
        if (temp == L->p_tail) L->p_tail = current;
        delete temp;
    }
}
...
bool addPos(List *&L, int data, int pos)
{
    ...
    if (current == L->p_tail) L->p_tail = newNode;
    return true;
}

void removePos(List *&L, int data, int pos)
{
    if (!L->p_head || pos < 0) return;
    if (pos == 0) {
    ...
    if (current && current->p_next && current->p_next->key == data) {
        NODE* temp = current->p_next;
        current->p_next = temp->p_next;
        if (temp == L->p_tail) L->p_tail = current;
        delete temp;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    if (!current->p_next) return false;
    NODE* newNode = createNode(data);
    if (!newNode) return false;
    newNode->p_next = current->p_next;
    current->p_next = newNode;
